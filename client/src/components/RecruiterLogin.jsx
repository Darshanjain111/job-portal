      ...
      :
       <>
        ...
        {state !== 'login' && ( <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
            ...
            <input className='outline-none text-sm' onChange={e => setName(e.target.value)} value = {name} type="text" placeholder="Complany Name" required />
        </div>)}
       ...
       {state === 'login' &&<p className='text-sm text-blue-600 mt-4 cursor-pointer'>Forgot Password?</p>}
        ...
        <button type='submit' className='bg-blue-600 w-full text-white py-2 rounded-full mt-4'>
            {state === 'login' ? 'login' : isTextDataSubmitted ? 'create accunt' : 'next'}
        </button>
{
    state === 'login'
    ?
